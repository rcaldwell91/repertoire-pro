...
(function () {
  ...
  /* ------------------------------------------------------------------ */
  /* An AVERAGE rather than a total, so stopping half way still places you on
     what you did say instead of throwing it away. Level 3 deliberately needs
     mostly top answers: it means every exercise unlocked and the proper terms
     with no hand-holding, which is not where "I know roughly" belongs. */
  function band(vals) {
    var got = vals.filter(function (v) { return v > 0; });
    if (!got.length) return null;
    var avg = got.reduce(function (a, b) { return a + b; }, 0) / got.length;
    return avg <= 1.8 ? 1 : avg < 3.34 ? 2 : 3;
  }

  ME.derive = function (a) {
    if (!a) return null;
    var sing   = band([+a.history || 0, +a.pitch || 0, +a.high || 0]);
    var taught = band([+a.terms || 0, +a.reading || 0, +a.key || 0]);

    /* The high-note question tells whoever answers "it hurts, or nothing
       comes out" that they get the gentle work first WHATEVER ELSE they
       answer. Without this line that sentence was not true: an otherwise
       confident singer averaged straight back up past it.

       Pain is not a level, it is a reason to start gently, and the app cannot
       tell the difference between a bad technique and a bad week. So it caps,
       and anybody who disagrees can move it in Profile or go and measure it. */
    if (+a.high === 1) sing = 1;
    if (sing == null && taught == null) return null;
    var t = taught == null ? ME.taught : taught;
    return {
      experience: sing == null ? ME.experience : sing,
      taught: t,
      words: t === 1 ? 'plain' : t === 2 ? 'some' : 'technical'
    };
  };
  ...
  function wireSkip(box) {
    ...
    if (!b) return;
    on(b, 'click', function () {
      /* Stopping half way KEEPS what was already said. Six questions is only
         reasonable because leaving after two is not punished — throwing those
         two away would make quitting cost something, which is the opposite of
         what "you don't have to answer these" means.

         Nothing is invented for the ones not reached: the average is taken
         over what was actually answered. */
      var got = Object.keys(answers || {}).length;
      if (got) {
      ...
      try {
        if (window.RP && RP.toast) {
          RP.toast(got ? 'Kept the ' + got + ' you answered. The rest is in Profile.'
                       : 'No problem. It is all in Profile if you change your mind.');
        }
      } catch (e) {}
    });
  }
  ...
  var QS = [
    ...
    { key: 'high', lab: 'YOUR SINGING',
      q: 'When you reach for a high note, what happens?',
      why: 'This one is about keeping you safe, not about how good you are.',
      opts: [
        [1, 'It hurts, or nothing comes out', 'Then you get the gentle work first, whatever else you answer.'],
        [2, 'It comes out, but it feels like shouting', 'Very common, and the thing most worth fixing.'],
        [3, 'A stretch, but it works', ''],
        [4, 'Comfortable \u2014 I know where my limit is', '']
      ] },
    ...
    { key: 'key', lab: 'WORDS AND MUSIC',
      q: 'Do you know what key a song is in while you are singing it?',
      why: '',
      opts: [
        [1, 'I do not know what that means', 'Fine. It is one of the first things the Learn tab explains.'],
        [2, 'I have heard of it, but I could not tell you', ''],
        [3, 'Usually, if I think about it', ''],
        [4, 'Yes \u2014 and I can move a song into another one', '']
      ] },
